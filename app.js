...
let weatherApiKey = localStorage.getItem('weather_api_key') || '';
...
async function handleSearch(e) {
    ...
    try {
        ...
        console.error(err);
        setText('location-status', "Ошибка поиска. Проверьте название города.");
    }
}
...
async function fetchWeatherData(lat, lon) {
    ...
    try {
        ...
        console.error(err);
        const errorMessage = err.message.includes("Key error") ? "Ошибка ключа WeatherAPI или лимита запросов." : "Ошибка провайдера погоды. Попробуйте другой источник или проверьте подключение.";
        setText('location-status', errorMessage);
    }
}
